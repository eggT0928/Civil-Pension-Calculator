...
def get_pension_start_age(entry_date: date, retirement_year: int) -> int:
    """1996.1.1 이후 임용자는 퇴직연도별 표를 적용 (법률 제13387호)"""
    ...
    return 60  # 1995.12.31 이전 임용자 경과규정 단순화
...
def calculate_pension(
    ...
    lump_sum_1_pv = actual_p1_value * y1 * lump_sum_multiplier if y1 > 0 else 0.0
    lump_sum_23_pv = actual_b_value * (y2 + y3) * lump_sum_multiplier if (y2 + y3) > 0 else 0.0
...
with st.sidebar:
    ...
    if use_exact_data:
        ...
        st.warning("⚠️ **추정 모드 한계:** 기여금만으로는 과거 소득 이력을 정확히 알 수 없어, 실제 연금액과 오차가 발생합니다.")
...
c1.metric("월 연금 (현재가치)", won(res["present_value_monthly_pension"]))
c2.metric("월 연금 (명목가치)", won(res["estimated_monthly_pension"]))
...
if use_exact_data:
    ...
    st.warning("⚠️ 현재 기여금만을 바탕으로 한 '추정 모드'입니다. 실제 서류가 있다면 사이드바에서 토글 스위치를 켜주세요.")
